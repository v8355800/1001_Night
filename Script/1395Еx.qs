...
function main() {
    switch (typeTest) {
        ...
        case 61: // Uпдmin - НЕрегулируемые
        {
            ...
            ApiDriver.KR_TurnOff();
            /* Сравнить результат с порогами, выйти из теста */
            if ((result >= minTest) && (result <= maxTest)) return RET_OK_A; // Годен
            else return RET_FAIL; // Брак
        ...
        case 7: // UсрEn
        {
            /* Добавить вспомогательную переменную Uout (float)*/
            var tempUout;
            /* Собрать схему */
            ApiDriver.KR_SetRele(UK_RELE_GND|UK_RELE_PWR|UK_RELE_ADJ_TO_OUT|UK_RELE_FTNCH2_TO_EN|UK_RELE_CAP_OUT|UK_RELE_CAP_IN);
            /* Установить реле МФТН */
            ...
            /* Получить мгновенное напряжение МЭН */
            tempUout = ApiDriver.EN_GetVoltage();
            /* Вычислить порог с шагом 1 мВ. Выпонять алгоритм внутри цикла,
               пока измеренное напряжение < userVar1  */
            while (tempUout < userVar1) {
                /* Если напряжение на выходе > userVar1 - выход */
                if (tempUout >= userVar1) break;
                /* Уменьшить напряжение МФТН на 1 мВ */
                lowSupplyVoltage = lowSupplyVoltage - 0.005;
                /* Если напряжение МФТН < minTest - выход */
        ...
        case 9: // UотпEn
        {
            ...
            while (tempUout > userVar1) {
                /* Если напряжение на выходе <= userVar1 - выход */
                if (tempUout <= userVar1) break;
                /* Увеличить напряжение МФТН на 1 мВ */
                lowSupplyVoltage += 0.005;
                /* Если напряжение МФТН > maxTest - выход */
        ...
        case 11: //IвхEn
        {
            /* Собрать схему */
            ApiDriver.KR_SetRele(UK_RELE_GND|UK_RELE_PWR|UK_RELE_ADJ_TO_OUT|UK_RELE_FTNCH2_TO_EN|UK_RELE_CAP_OUT|UK_RELE_CAP_IN);
            /* Установить реле МФТН (с шунтом 10 мА и измерителем) */
            ...
            /* Подождать установки параметров */
            ApiDriver.delayms(15);
            /* Установить реле МФТН (без измерителя и шунта) */
	...
	case 121: //Iпот через МФТН для (044003)
        {
            ...
            /* Начать измерение параметров МФТН с выборкой (фильтром) */
            ApiDriver.MU_SetOutputBit(MU_OUT8);
            ApiDriver.FTN_StartBuffer(FTN_CH1, timeMeas);
            /* Подождать окончания измерения */
            ApiDriver.delayms(timeMeas);
            /* Считать ток МФТН после окончания измерения и записать в переменную «result» */
            result = ApiDriver.FTN_GetBufferI(FTN_CH1);
            /* Определить диапазон измерения, если ток больше 0.11 мА, то включить шунт и произвести измерение снова */
            if (result > 0.11) 
            {
            ...
            /* Выключить МКР */
            ApiDriver.KR_TurnOff();
            /* Перевести мА в мкА и изменить знак */
            result = result * (-1000.0);
            /* Сравнить результат с порогами, выйти из теста */
            if ((result >= minTest) && (result <= maxTest)) return RET_OK; // Годен
            else return RET_FAIL; // Брак
        ...
        case 122: //Iпот.В через МФТН (для 044003)
        {
            /* Собрать схему */
            ApiDriver.KR_SetRele(UK_RELE_PWR|UK_RELE_ADJ_TO_OUT|UK_RELE_FTNCH2_TO_EN|(1<<15)|UK_RELE_CAP_OUT|UK_RELE_CAP_IN);
            /* Установить реле МФТН (без измерителя) */
        ...
        case 16: //Ki
        {
            ...
            currI1 = loadCurrent;
            /* Установить ток нагрузки ЭН userVar1 */
